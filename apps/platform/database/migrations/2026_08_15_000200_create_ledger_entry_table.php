...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if ($driver === 'mysql' || $driver === 'mariadb') {
            // REQ-NFR-032 — monthly range partitions for per-state remittance queries at
            // volume. Partitioning must be declared at creation; MySQL requires the
            // partition key be part of every unique key, and this table has none besides
            // the auto id, so a simple RANGE COLUMNS on createdAt applies cleanly.
            DB::statement(<<<'SQL'
