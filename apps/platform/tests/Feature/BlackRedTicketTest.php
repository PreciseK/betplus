...
final class BlackRedTicketTest extends TestCase
{
    ...
    public function test_ticket_purchase_no_longer_auto_funds_from_opay_when_play_balance_is_zero(): void
    {
        // Ticket creation has no wallet-auto-fund fallback any more — funding is the
        // caller's job (the web deposit flow, or the USSD direct-pay OTP flow), never
        // an implicit side effect of trying to place a bet with no idempotencyKey-
        // string-prefix magic. An insufficient balance just fails cleanly.
        [, $token] = $this->signedInPlayer(fundedKobo: 0);

        $response = $this->withToken($token)->postJson('/v1/tickets', [
            'prediction' => ['B', 'R'],
            'stake_kobo' => 50_000,
            'idempotency_key' => 'ussd-br-sess-no-autofund',
        ]);
